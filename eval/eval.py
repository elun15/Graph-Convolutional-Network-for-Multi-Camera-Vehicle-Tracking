...
def eval(test, pred,th,  **kwargs):
    ...
    def compare_dataframes_mtmc(gts, ts, th):
        ...
        # th = 0.5
        multiCamAcc = mm.utils.compare_to_groundtruth(pd.concat(gtds), pd.concat(tsds), 'iou',distth=th)
    ...
    # evaluate results
    return compare_dataframes_mtmc(test, pred,th)
...
if __name__ == '__main__':
    ...
    try:
        ...
        summary.to_excel(os.path.join(os.path.split(args.data[1])[0],'metrics_' + os.path.basename(os.path.splitext(args.data[1])[0]) +'.xls'), index=False)
        summary.to_csv(os.path.join(os.path.split(args.data[1])[0],'metrics_' + os.path.basename(os.path.splitext(args.data[1])[0]) + '.csv'), index=False)
